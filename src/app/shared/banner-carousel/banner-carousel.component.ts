...
export class BannerCarouselComponent implements OnInit, AfterViewInit {
  ...
  /**
   * Navegar a una URL de forma reactiva
   * @param url URL de destino
   */
  openLink(url: string): void {
    if (!url) {
      console.warn('🔗 BANNER: URL vacía, no se puede navegar');
      return;
    }

    console.log('🔗 BANNER: Navegando a URL:', url);

    // Si la URL es externa (comienza con http/https) o contiene dominio diferente
    if (this.isExternalUrl(url)) {
      console.log(
        '🌐 BANNER: URL externa detectada, abriendo en nueva pestaña'
      );
      window.open(url, '_blank');
      return;
    }

    // Si la URL es interna, usar Router para navegación reactiva
    try {
      // Limpiar la URL para navegación interna
      const internalUrl = this.cleanInternalUrl(url);
      console.log('📱 BANNER: Navegación reactiva a:', internalUrl);
      this.router.navigate([internalUrl]);
    } catch (error) {
      console.error('❌ BANNER: Error en navegación reactiva:', error);
      // Fallback a navegación tradicional
      console.log('🔄 BANNER: Usando fallback de navegación tradicional');
   ...
   * @returns true si es externa, false si es interna
   */
  ...
  /**
   * Limpiar URL interna para navegación con Router
   * @param url URL a limpiar
   * @returns URL limpia para Router
   */
  private cleanInternalUrl(url: string): string {
    // Remover barra inicial si existe
    let cleanUrl = url.startsWith('/') ? url.substring(1) : url;

    // Si está vacía, ir al home
    if (!cleanUrl) {
      cleanUrl = '';
    }
